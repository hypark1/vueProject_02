...
export default {
  ...
  methods: {
    ...
    tableRowClick (v) {
      this.$router.push({ name: 'revenueHourly', query: { date: v.daily } })
    },
    submitDate () {
      ...
      if (startNum > endNum) {
        ...
        this.$router.push({ 'name': 'revenueDaily', query: { start: start, end: end } })
        this.reFresh()
      }
    },
    excelDownload () {
    ...
    getRevenueChart (send) {
      this.xhttp({
        url: '/reports/ads/daily/revenue-chart',
        method: 'get',
        params: send
      }).then((response) => {
    ...
    getRevenueList (send) {
      this.xhttp({
        url: '/reports/ads/daily/revenue-list',
        method: 'get',
        params: send
      }).then((response) => {
        if (response.status === 200) {

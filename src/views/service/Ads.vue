            ...
            <template v-slot:item="{ item }">
              <tr style="background: #ddd;">
                ...
                <td class="text-right" style="width:170px">
                  <template v-if="item.ctr !== null">{{ numReplace(item.ctr) }}%</template>
                </td>
                <td class="text-right" style="width:170px">
                  <template v-if="item.video_seconds !== null">{{ timeSet(item.video_seconds) }}</template>
                </td>
                <td class="text-right" style="width:170px">
                  <template v-if="item.vctr !== null">{{ numReplace(item.vctr) }}%</template>
                </td>
                <td class="text-right" style="width:170px">
                  <template v-if="item.rpm !== null">{{ numReplace(item.rpm) }}원</template>
                </td>
                <td class="text-right" style="width:175px">
                  <template v-if="item.click_revenue !== null">{{ numReplace(item.click_revenue) }}원</template>
                </td>
              </tr>
            </template>
          </v-data-table>
  ...
  methods: {
    ...
    submitDate () {
      ...
      if (startNum > endNum) {
        ...
        this.$router.push({ 'name': 'serviceAds', query: { start: start, end: end } })
        this.reFresh()
      }
    },
    excelDownload () {
      ...
      let file = this.$store.state.baseURL + '/reports/ads/daily/revenue-excel?sdate=' + this.datePicker.start.date + '&edate=' + this.datePicker.end.date
      ...
      fetch(file, { headers })
        ...
        .then(blobby => {
          ...
          anchor.download = this.datePicker.start.date + '_' + this.datePicker.end.date + '_AdsReport_daily.xlsx'
          anchor.click()
    ...
    tableRowClick (v) {
      this.$router.push({ name: 'serviceAdsHourly', params: { date: v.daily } })
    },
    lineTapClick (v) {
    ...
    getAdsChart (send) {
      this.xhttp({
        ...
        if (response.status === 200) {
          ...
          data1.dataSource.data = this.response.adsChart.data.chart[0]

            ...
            <template v-slot:item="{ item }">
              <tr style="background: #ddd;">
                ...
                <td class="text-right" style="width:170px">
                  <template v-if="item.feed_seconds !== null">{{ timeSet(item.feed_seconds) }}</template>
                </td>
                <td class="text-right" style="width:170px">
                  <template v-if="item.avg_feed_seconds !== null">{{ timeSet(item.avg_feed_seconds) }}</template>
                </td>
  ...
  methods: {
    ...
    submitDate () {
      ...
      if (startNum > endNum) {
        ...
        this.$router.push({ 'name': 'serviceMedia', query: { start: start, end: end } })
        this.reFresh()
      }
    },
    excelDownload () {
      ...
      let file = this.$store.state.baseURL + '/reports/media/daily/excel?sdate=' + this.datePicker.start.date + '&edate=' + this.datePicker.end.date
      ...
      fetch(file, { headers })
        ...
        .then(blobby => {
          ...
          anchor.download = this.datePicker.start.date + '_ServiceReport_hourly.xlsx'
          anchor.click()
    ...
    tableRowClick (v) {
      this.$router.push({ name: 'serviceMediaHourly', params: { date: v.daily } })
    },
    lineTapClick (v) {

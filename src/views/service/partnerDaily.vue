...
export default {
  ...
  methods: {
    ...
    excelDownload () {
      ...
      let file = this.$store.state.baseURL + '/reports/ads/daily/cp-excel?sdate=' + this.datePicker.start.date + '&edate=' + this.datePicker.end.date
    ...
    getRevenueChart (send) {
      this.xhttp({
        url: '/reports/ads/daily/cp-chart',
        method: 'get',
        params: send
      }).then((response) => {

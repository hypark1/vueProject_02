...
export default {
  ...
  methods: {
    ...
    submitDate () {
      ...
      this.$router.push({ 'name': 'reportHourly', query: { date: start } })
      this.reFresh()
    },
    excelDownload () {
      let anchor = document.createElement('a')
      document.body.appendChild(anchor)
      let file = this.$store.state.baseURL + '/reports/ads/hourly/cp-excel?sdate=' + this.datePicker.start.date + '&edate=' + this.datePicker.end.date
    ...
    getRevenueChart (send) {
      this.xhttp({
        url: '/reports/ads/hourly/cp-chart',
        method: 'get',
        params: send
      }).then((response) => {
    ...
    getRevenueList (send) {
      this.xhttp({
        url: '/reports/ads/hourly/cp-list',
        method: 'get',
        params: send
      }).then((response) => {
        if (response.status === 200) {

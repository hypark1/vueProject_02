  ...
  methods: {
    ...
    submitDate () {
      ...
      if (startNum > endNum) {
        ...
        this.$router.push({ 'name': 'serviceCategory', query: { start: start, end: end } })
        this.reFresh()
      }
    },
    excelDownload () {
      ...
      fetch(file, { headers })
        ...
        .then(blobby => {
          ...
          anchor.download = this.datePicker.start.date + '_' + this.datePicker.end.date + '_CategoryReport_daily.xlsx'
          anchor.click()
    ...
    tableRowClick (v) {
      this.$router.push({ name: 'serviceCategoryHourly', params: { date: v.daily } })
    },
    lineTapClick (v) {

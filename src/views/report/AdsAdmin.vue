              ...
              <v-row>
                ...
                <v-col>
                  <v-card outlined class="pa-3">
                    <v-card-text class="justify-center font-weight-regular text-center">
                      <h3>총 매출</h3><br>
                      <h1>{{numReplace(totalReplace(resultReport.card.total_revenue, resultReport.card.total_media_revenue, resultReport.card.total_content_revenue  ))}}원</h1>
                    </v-card-text>
                  </v-card>
                </v-col>
                <v-col>
                ...
                <v-data-table
                  ...
                  <template v-slot:item="{ item }">
                    <tr @click="tableRowClick(item)">
                      ...
                      <td class="text-center" style="width:270px">{{ numReplace(item.views) }}</td>
                      <td class="text-center" style="width:270px">{{ numReplace(totalReplace(item.revenue, item.media_revenue, item.content_revenue)) }}원</td>
                      <td class="text-center" style="width:270px">{{ numReplace(item.revenue) }}원</td>
                      <td class="text-center" style="width:270px">{{ numReplace(item.media_revenue) }}원</td>
                      <td class="text-center" style="width:270px">{{ numReplace(item.content_revenue) }}원</td>
                    </tr>
                  </template>
                </v-data-table>
              </v-col>
  ...
  methods: {
    ...
    submitDate (val) {
      ...
      if (startNum > endNum) {
        ...
        this.$router.push({ 'name': 'reportAds', query: { start: start, end: end } })
        this.reFresh()
      }
    },
    tableRowClick (v) {
      this.$router.push({ name: 'reportAdsHourly', params: { date: v.daily } })
    },
    reFresh () {

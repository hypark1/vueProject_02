              ...
              <v-row>
                <v-col cols="2">
                  ...
                  <v-card outlined class="pa-3">
                    <v-card-text class="justify-center font-weight-regular text-center">
                      <h3>뷰당 단가</h3><br>
                      <h1>{{toFixed(resultDashboard.card.total_content_revenue, resultDashboard.card.total_views)}}원</h1>
                    </v-card-text>
                  </v-card>
                  <v-card outlined class="pa-3">
                    <v-card-text class="justify-center font-weight-regular text-center">
...
export default {
  ...
  methods: {
    ...
    getDashboard (send) {
      this.xhttp({
        url: '/reports/dashboard/cp',
        method: 'get',
        params: send
      }).then((response) => {
        if (response.status === 200) {
          this.response.dashboard = response.data.data
          if (this.response.dashboard.list.length > 0) {

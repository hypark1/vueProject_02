              ...
              <v-data-table
                ...
                <template v-slot:item="{ item }">
                  <tr style="background: #ddd;">
                    ...
                    <td class="text-right" style="width:190px;padding-right:25px;">{{ numReplace(item.cpc) }}원</td>
                  </tr>
                </template>
              </v-data-table>
            </v-col>
...
export default {
  ...
  methods: {
    ...
    excelDownload () {
      ...
      let file = this.$store.state.baseURL + '/reports/content/hourly/cp-excel/' + this.$route.params.contents_id + '?sdate=' + this.datePicker.start.date + '&edate=' + this.datePicker.start.date
    ...
    getContentChart (send) {
      ...
      this.xhttp({
        url: '/reports/content/hourly/cp-chart/' + id,
        method: 'get',
        params: send
      }).then((response) => {
    ...
    getContentList (send) {
      let id = this.$route.params.contents_id
      this.xhttp({
        url: '/reports/content/hourly/cp-list/' + id,
        method: 'get',
        params: send
      }).then((response) => {
        if (response.status === 200) {
    ...
    tableRowClick (v) {
      this.$router.push({ name: 'partnersContentsHourly', params: { date: v.daily } })
    },
    exposure () {
      let text = '게재'
...
<style>
  #contentsViewHourly .resultReport tbody tr:first-child {
